...
public:
	...
	Impute(){	rng=gsl_rng_alloc(gsl_rng_default);	gsl_rng_set(rng,	time(NULL));	}  // set default seed?
...
bool	Impute::load_bin(const	char	*F){
	...
	while(gzgets(f,	buffer,	1024*1024)!=NULL){
		...
		site.push_back(temp);	posi.push_back(temp.pos);  // add to end of vector
		for(uint	i=0;	i<2*in;	i++){	si>>rawp;	prob.push_back(rawp);	}  // store the raw GL , which the prob of het and homo alt
	}
...
uint	Impute::load_vcf(const	char	*F){  // this section loads known genotypes
	...
	uint	vin;
	{
		...
		vin=vname.size();	vid.resize(vin);  //vin is vname size.....  which stores something from tabix
	...
	while((s=ti_read(t,	iter,	&len))!=0){
		...
		si>>st>>st>>st>>st;
		for(uint	m=0;	m<mn;	m++)	if(site[m].chr==chr&&site[m].pos==pos&&site[m].all==all){  // for each element in site vector
			for(uint	i=0;	i<vin;	i++){  
...
void	Impute::initialize(void){  
	...
	hn=in*2;	haps.resize(hn*wn);	hnew.resize(hn*wn);  // number of haplotypes = 2 * number of samples  ... haps mn is # of sites, 
	hsum.assign(hn*mn,	0);	pare.assign(in*in,	0);  // set to zeros
	pn=3*mn;	tran.resize(pn);	vector<fast>	temp(in*pn);	// transitions between 3 types of genotypes P(RR), P(RA) P(AA)
	en=4*mn;	emit.resize(in*en);	  // 4 emissions, for each site 4 emissions * number of samples.  (0|0, 1|1 0|1 1|0)
	...
	if(posi.size()!=mn){	posi.resize(mn);	for(uint	m=0;	m<mn;	m++)	posi[m]=m;	}    // if sites not stored
	mu=1/mu;	rho=0.5*mu*(mn-1)/(posi[mn-1]-posi[0])/density;	mu=mu/(hn+mu);  // rho is recombination rate?  mu is mutation rate
	for(uint	m=mn-1;	m;	m--){
		posi[m]=(posi[m]-posi[m-1])*rho;	fast	r=posi[m]/(posi[m]+hn); // posi is recombination between its position and previous, transition is  
		tran[m*3]=(1-r)*(1-r);	tran[m*3+1]=r*(1-r);	tran[m*3+2]=r*r;  // for each position, transition.  r= alternative, 1-r= refrence? 4 state HMM with three transitions at each position
	}
	...
	for(uint	i=0;	i<in;	i++){
		...
		for(uint	m=0;	m<mn;	m++,	t+=3,	e+=4,	p+=hn){
			...
			if(is_x&&male.find(name[i])!=male.end()&&!is_par[m]){ /// treat it differently for genders
				...
				t[0]=fmaxf(1-p[0]-p[1],0.0f);	t[1]=fmaxf(p[0],0.0f);	t[2]=fmaxf(p[1],0.0f);  // initial prob is the GL.  
			}
			for(uint	j=0;	j<4;	j++)  
...
fast	Impute::hmm_like(uint	I,	uint	*P){
	...
	for(uint	m=1;	m<mn;	m++,	e+=4,	t+=3){
		...
		l10=b10*e[(test(f1,m)<<1)|test(m0,m)];	l11=b11*e[(test(f1,m)<<1)|test(m1,m)];  // the test section should return a value between 0 and 3.  
...
void	Impute::estimate(void){
	...
	for(uint	n=0;	n<bn+sn;	n++){  // n is number of cycles
		fast	sum=0,	pen=fminf(2*(n+1.0f)/bn,	1),	iter=0;	pen*=pen;  // pen = 1 
		for(uint	i=0;	i<in;	i++){
			uint	len=nn*in;  // nn is number of folds
