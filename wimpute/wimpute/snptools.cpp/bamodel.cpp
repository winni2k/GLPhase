...
struct	Site{ // data structure for Site....
	char	chr[10],	all[2];
	uint	pos;
...
struct	bam_file{
	...
	char	**name;	//	pointer to pointer of chromosome names
	uint32_t	sn;	//	number of sites to consider
	vector<float>	ebd;  //	effective base depth at each site
...
float	bam_file::phred[256];
 ...
 // recalculates the ebd values-- not exactly the same code... because we have predefined the ref and alt now
...
bool	bam_file::scan_bam(void){
	samfile_t *bam;  // pointer to bam file.... 
	bam=samopen(file.c_str(), "rb", 0);  // open file read binary bam
	if(bam==0)	return	false;
	cerr<<"scaning\t"<<file<<endl;
	name=bam->header->target_name;
	ebd.assign(sn*2,	0);
	sampileup(bam,	-1,	pileup_func,	this);  // create ebd
	samclose(bam);
	return	true;
}
...
bool	bamodel::load_site(const	char	*F){
	string	temp,	a,	b;
	ifstream	fi(F);	if(!fi)	return	false;  // reads in the file as an inpute stream
	cerr<<"loading\t"<<F<<endl;
	Site	s;
	for(getline(fi,	temp);	!fi.eof();	getline(fi,	temp))	if(temp.size()&&temp[0]!='#'){
		istringstream	si(temp);
		memset(&s,	0,	sizeof(Site));
		si>>a;	strncpy(s.chr,	a.c_str(),	9);  // stores the position of the snp
		si>>s.pos>>a>>a>>b;	s.all[0]=a[0];	s.all[1]=b[0];  // stores allele 0 and 1
		site.push_back(s);
	}
	fi.close();
...
bool	bamodel::scan_bams(void){
	bn=bam.size();  // returns number of bams
	for(uint	i=0;	i<bn;	i++){
		bam[i].site=&site[0];	bam[i].sn=sn;	bam[i].ebd.resize(2*sn);
		if(!bam[i].scan_bam()){	cerr<<"fail to scan "<<bam[i].file<<endl;	return	false;	}  // scan each bam calculate the ebd for each file
	}
	return	true;
}

void	bamodel::estimate(const	char	*F){
	double	na=0.91,	nh=0.05,	nb=0.04,	oa,	ob,	oh,	delta;  // what is na, nh and nb--- equals 100%.. percentage of each type?
	vector<double>	ea(bn),	eh(bn),	eb(bn);
	vector<double>	lap(bn),	laq(bn),	lhp(bn),	lhq(bn),	lbp(bn),	lbq(bn); // declare vectors of length bn; lop
	vector<double>	sap(bn),	saq(bn),	shp(bn),	shq(bn),	sbp(bn),	sbq(bn); // what is this?
	
	for(uint	i=0;	i<bn;	i++){	ea[i]=0.99;	eh[i]=0.5;	eb[i]=0.02;	}  // prior probabilities, probability of alternative | g= alternative
	cout.precision(4);	cout.setf(ios::fixed);
	do{
		double	la=log(na),	lh=log(nh),	lb=log(nb);  
		oa=na;	oh=nh;	ob=nb;	na=nh=nb=0;
		for(uint	i=0;	i<bn;	i++){
			lap[i]=log(ea[i]);	laq[i]=log(1-ea[i]);  // log p + log (1-p)
			lhp[i]=log(eh[i]);	lhq[i]=log(1-eh[i]);
			lbp[i]=log(eb[i]);	lbq[i]=log(1-eb[i]);
			sap[i]=saq[i]=shp[i]=shq[i]=sbp[i]=sbq[i]=0; // this is the expected value N*p for each possiblity, sap= alternative p, saq= alternative (1-p)
		}
		for(uint	m=0;	m<sn;	m++){   // for each site-- not the same as for length of bam
			uint	p=m*2;
			double	wa=la,	wh=lh,	wb=lb;  // log of reference, alther
			for(uint	i=0;	i<bn;	i++){
				double	ta=bam[i].ebd[p],	tb=bam[i].ebd[p+1]; // how long is this EBD array--- is it just ebd of reference vs alternative?
				wa+=ta*lap[i]+tb*laq[i];	wh+=ta*lhp[i]+tb*lhq[i];	wb+=ta*lbp[i]+tb*lbq[i];
			}
			if(wa>=wh&&wa>=wb){	wh=exp(wh-wa);	wb=exp(wb-wa);	wa=1;	}  // if w_a is largest, then calculate relative difference in probability 
			else	if(wh>=wa&&wh>=wb){	wa=exp(wa-wh);	wb=exp(wb-wh);	wh=1;	}
			else{	wa=exp(wa-wb);	wh=exp(wh-wb);	wb=1;	}
			double	sum=1.0/(wa+wh+wb);	wa*=sum;	wh*=sum;	wb*=sum;  // rescale all parameters
			na+=wa;	nh+=wh;	nb+=wb;  // update percentage of each type
			for(uint	i=0;	i<bn;	i++){
				double	ta=bam[i].ebd[p],	tb=bam[i].ebd[p+1];
				sap[i]+=ta*wa;	saq[i]+=tb*wa;	shp[i]+=ta*wh;	shq[i]+=tb*wh;	sbp[i]+=ta*wb;	sbq[i]+=tb*wb;  
			}
		}
		double	sum=1.0/(na+nh+nb+1); // rescale?  isn't this just 1/2?
		na=(na+0.91)*sum;	nh=(nh+0.05)*sum;	nb=(nb+0.04)*sum; // basically only adjust by halves
		cout<<"Ref/Ref\tRef/Alt\tAlt/Alt\tFile\n";
		for(uint	i=0;	i<bn;	i++){  // update parameters
		...
		delta=sqrt((na-oa)*(na-oa)+(nh-oh)*(nh-oh)+(nb-ob)*(nb-ob));  // criteria calculation
	}while(delta>1e-6);
	
	string	s=F;	s+=".emit";
	FILE	*f=fopen(s.c_str(),	"wt");
	for(uint	i=0;	i<bn;	i++)	fprintf(f,	"%s\t%g\t%g\t%g\n",	bam[i].file.c_str(),	ea[i],	eh[i],	eb[i]);  // emission values
	fclose(f);
	
	s=F;	s+=".weig";
	f=fopen(s.c_str(),	"wt");
	fprintf(f,	"%s\t%g\t%g\t%g\n",	F,	na,	nh,	nb);  // relative weights of each of the possible genotypes... not weights but probabilities. 
	fclose(f);
	...
	for(uint	m=0;	m<sn;	m++){
		...
		uint16_t	d[2]={(uint16_t)(wh*sum+0.5),	(uint16_t)(wb*sum+0.5)};  // write raw probability
		fwrite(d,	4,	1,	f);
	}
	fclose(f);
}
...
// Main program.  Setup Phred Scoring and priors.  
...
// for each option in... ie for all bams input
	for(int	i=optind+2;	i<ac;	i++)	bm.bam[i-optind-2].file=av[i];
	if(!bm.load_site(av[optind+1])){	cerr<<"fail to load\t"<<av[optind]<<endl;	return	0;	} // load the site list
	if(!bm.scan_bams())	return	0;    // scan the bam
