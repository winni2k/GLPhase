...
HMM::HMM(HMMHelper::Init init, const std::vector<fast> &transitions_,
         const std::vector<fast> &emissions_,
         const std::vector<uint64_t> &haps_)
    : m_init(move(init)), transitions(transitions_), emissions(emissions_),
      haps(haps_) {
...
vector<fast> HMM::alphas(unsigned sample_number, uint *proposal_haps) {
  ...
  const uint64_t *f0 = &haps[proposal_haps[0] * m_init.hap_size_in_blocks],
                 *f1 = &haps[proposal_haps[1] * m_init.hap_size_in_blocks],
                 *m0 = &haps[proposal_haps[2] * m_init.hap_size_in_blocks],
                 *m1 = &haps[proposal_haps[3] * m_init.hap_size_in_blocks];

  vector<fast> alphas;
  alphas.reserve(m_init.num_sites * 4);

  // pull out phase emission and transition probabilities
  const fast *emit_pointer =
                 &emissions[sample_number * m_init.num_possible_emissions],
  ...
  for (unsigned m = 1; m < m_init.num_sites;
       m++, emit_pointer += 4, tran_pointer += 3) {
...
size_t HMM::sample_z(std::vector<fast>::iterator alpha_begin,
  ...
  const *fast tran_pointer = &transissions[first_transission];
...
size_t HMM::sample_y(size_t sampled_z_alleles, const fast *prob_pointer) {
  ...
  vector<fast> vals;
  for (int y = 0; y < 4; ++y) {
    vals.push_back(prob_pointer[to_gt(y)] *
                   pc[sampled_z_alleles][y]);
  }
