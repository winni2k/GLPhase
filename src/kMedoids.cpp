...
void KMedoids::init(const vector< uint64_t > * pvuHaplotypes,
...
void KMedoids::AssignHapsToBestMedoids(const vector< uint64_t > *
                                       pvuHaplotypes) {
    ...
    for (unsigned uHapNum = 0; uHapNum < m_vuHapMedNum.size(); uHapNum++) {
        ...
        unsigned uHamming = hTester.HammingDist(&(*pvuHaplotypes)[uHapNum * m_uNumWordsPerHap],
        ...
        for (unsigned uMedNum = 1; uMedNum < m_vuMedoidHapNum.size(); uMedNum ++) {
            ...
            unsigned uNewHamming = hTester.HammingDist(&(*pvuHaplotypes)[uHapNum * m_uNumWordsPerHap],
...
unsigned KMedoids::SampleHap(unsigned uInd, gsl_rng *rng) {
    ...
    assert(vuHapsOfInterest.size() >
           2); // this avoids nasty business of only sampling haps from the individual of interest

    unsigned uPropHap =
        m_uNumHaps; // this should be out of bounds if left unchanged
