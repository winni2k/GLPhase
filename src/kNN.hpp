...
private:
  ...
  bool UsingScaffold() { return (m_dFreqCutoff >= 0 && m_dFreqCutoff <= 1); }

public:
  KNN(unsigned uNumClust, kNNDistT distMetric = kNNDistT::hamming)
      : m_uNumClusters(uNumClust), m_distMetric(distMetric) {};

  void init(const std::vector<uint64_t> &vuHaplotypes, unsigned uNumWordsPerHap,
