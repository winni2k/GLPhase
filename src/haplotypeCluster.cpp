...
haplotypeCluster::haplotypeCluster(genhap_set  * g) {
	this->g = g;
}
...
void haplotypeCluster::build(int a,int b,float c1,float c2,int ncond) {
	...
	bifurc_kmeans(); //divisive K-means clustering routine - my current preference
	topupClusters();//ensures cluster size >= min_cluster_size

	for(map<unsigned int,vector<int> >::iterator it=clusters.begin(); it!=clusters.end();it++)
		assert(it->second.size()>=min_cluster_size);

	if(_DEBUG>0) summariseKmeans();
}
...
set<int> sampleIndex(int k,int n) {
	...
	for(int i=0;i<k;i++) {
		...
		while(ret.count(idx))  idx = putils::getRandom(n);
		ret.insert(idx);
	}
	return(ret);
}
...
int haplotypeCluster::topupClusters() {
	if(_DEBUG>1) cout << "Topping up clusters." <<endl;
	for(map<unsigned int,vector<int> >::iterator it=clusters.begin(); it!=clusters.end();it++) {
		if(_DEBUG>5)		cout << it->first << " " << it->second.size() << endl;
		if(it->second.size() < min_cluster_size ) {
if(_DEBUG>5)			cout << "Topping up " << it->first << endl;
