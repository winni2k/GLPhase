...
private:
    ...
    float euc(int i,int j, vector< vector<float> > & mu, vector< vector< vector<float> > > & dlook);
    pair<int,int> kmeans(unsigned int cluster,unsigned int depth);
    int bifurc_kmeans();
    int bifurc_kmeans(unsigned int cluster,int n,int depth);
    int getIdx(int idx);
    int updateMeans(pair<unsigned int,unsigned int> & cluster_id,pair<int,int> &cluster_size);
